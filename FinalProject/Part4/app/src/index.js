...
const App = {
    ...
    openPosition: async function() {
        ...
        await this.meta['Mint'].methods.openPosition(deposit, this.meta[sAsset].options.address, CR).send({ from: this.accounts[0]});
        this.refreshBalance();
        this.setStatus("Transaction complete!");
    },
    ...
    addLiquidity: async function() {
        ...
        // const reserves = await this.meta['Swap'].methods.getReserves().send({ from: this.accounts[0]});
        ...
        // const l0 = parseInt(document.getElementById("liquidity0").value * 10**8);
        // const l1 = parseInt(document.getElementById("liquidity1").value * 10**8);
        // if (l0 == l1){
        //     return;
        // }
        // console.log(l1);
    ...
    token0To1: async function() {
        this.setStatus("Initiating transaction... (please wait)");
        const swapAmount = parseInt(document.getElementById("swap0").value * 10**8).toString();
        await this.approve('sBNB', this.meta['Swap'].options.address, swapAmount);
        await this.meta['Swap'].methods.token0To1(swapAmount).send({ from: this.accounts[0 ]});
        this.refreshBalance();
        this.setStatus("Transaction complete!");
    },
    ...
    token1To0: async function() {
        this.setStatus("Initiating transaction... (please wait)");
        const swapAmount = parseInt(document.getElementById("swap1").value * 10**8).toString();
        await this.approve('sTSLA', this.meta['Swap'].options.address, swapAmount);
        await this.meta['Swap'].methods.token1To0(swapAmount).send({ from: this.accounts[0 ]});
        this.refreshBalance();
        this.setStatus("Transaction complete!");
    },
